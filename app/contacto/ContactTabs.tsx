...
const sedes = [
  ...
  },
  {
    ...
    direccion: "Anexo 22 Jicamarca Mz AW Lt 06\nAsc. de Pob. El Cercado - Huarochiri San Antonio",
    telefono: "977 928 236",
    mapUrl: "https://maps.app.goo.gl/genericLink2",
    embedSrc: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3903.454278453582!2d-76.9935105!3d-11.9065112!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x9105c3d2296495cb%3A0x280d8531185489f6!2sJicamarca!5e0!3m2!1ses-419!2spe!4v1707920000000!5m2!1ses-419!2spe",
    pinLabel: "Jicamarca",
  },
  {
    num: 3,
...
export default function ContactTabs() {
  const [activeTab, setActiveTab] = useState<"formulario" | "ubicacion">("formulario");
    ...
    if (nombre && celular && producto) {
      ...
      formRef.current.reset();
       ...
       setModal({
        ...
        type: 'warning',
        title: 'Faltan datos',
        message: 'Por favor completa los campos para poder generar el enlace de WhatsApp.'
      });
    }
  };
  ...
  const handleEmailClick = async () => {
    ...
    if (!nombre || !celular || !producto) {
      setModal({
        ...
        message: 'Por favor completa los campos obligatorios (*) marcados en el formulario.'
      });
      return;
    }
    ...
    try {
      ...
      setModal({
        ...
        message: 'Hemos recibido tu solicitud correctamente. Nos pondremos en contacto contigo a la brevedad.'
      });
      ...
      setModal({
        ...
        message: 'Hubo un problema al enviar el correo. Por favor intenta mediante el botón de WhatsApp.'
      });
    } finally {
      setIsSending(false);
    }
  ...
  return (
    <div className="bg-white rounded-2xl md:rounded-3xl shadow-2xl overflow-hidden max-w-5xl mx-auto border border-gray-100 relative">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-white/90 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-8 max-w-sm w-full text-center transform scale-100 animate-in zoom-in-95 duration-200">
            <div className="flex justify-center mb-4">
              {modal.type === 'success' && <FaCheckCircle className="text-5xl text-green-500" />}
              {modal.type === 'error' && <FaTimesCircle className="text-5xl text-red-500" />}
              {modal.type === 'warning' && <FaExclamationCircle className="text-5xl text-brand-accent-yellow" />}
            </div>
            ...
            <button 
              ...
              style={{ backgroundColor: modal.type === 'error' ? '#EF4444' : '' }}
            >
              Entendido
            </button>
          </div>
          ...
          <form ref={formRef} className="space-y-4 md:space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input name="nombre" type="text" id="nombrePage" required className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-brand-primary outline-none text-sm md:text-base" placeholder="Tu nombre" />
              </div>
              <div>
                <label htmlFor="celularPage" className="block text-xs md:text-sm font-bold text-brand-dark mb-2">Celular *</label>
                <input name="celular" type="tel" id="celularPage" required className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-brand-primary outline-none text-sm md:text-base" placeholder="999 888 777" />
              </div>
            </div>
            
            <div className="bg-brand-accent-yellow/5 p-4 rounded-lg border border-brand-accent-yellow/20">
              <label htmlFor="productoPage" className="block text-xs md:text-sm font-bold text-brand-dark mb-2">¿Qué producto necesitas? *</label>
              <input name="producto" type="text" id="productoPage" required className="w-full px-4 py-3 rounded-lg bg-white border border-gray-200 focus:border-brand-primary outline-none text-sm md:text-base" placeholder="Ej: Ladrillos King Kong" />
            </div>

            <div>
              <label htmlFor="mensajePage" className="block text-xs md:text-sm font-bold text-brand-dark mb-2">Mensaje Adicional</label>
              <textarea name="mensaje" id="mensajePage" rows={3} className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-brand-primary outline-none text-sm md:text-base resize-none" placeholder="Detalles extra..."></textarea>
            </div>
      ...
      {/* CONTENIDO UBICACIÓN CON MAPA */}
      <div className={`p-6 md:p-12 ${activeTab === "ubicacion" ? "block" : "hidden"}`}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {sedes.map((sede) => (
                <div key={sede.num} className="bg-gray-50 p-6 rounded-xl border border-gray-100 hover:shadow-lg transition-shadow">
                    <div className={`inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-3 ${sede.color === 'brand-primary' ? 'bg-blue-600' : sede.color}`}>
                        Sede {sede.num}
                    </div>
                    <h4 className="font-bold text-lg text-brand-dark mb-2">{sede.nombre}</h4>
                    <p className="text-brand-grey text-sm mb-2 flex gap-2">
                        <FaMapLocationDot className="mt-1 text-brand-primary"/> {sede.direccion}
                    </p>
                    <p className="text-brand-grey text-sm mb-4 flex gap-2">
                        <FaPhone className="mt-1 text-brand-primary"/> {sede.telefono}
                    </p>
                    <a href={sede.mapUrl} target="_blank" rel="noreferrer" className="text-brand-primary font-bold text-sm underline decoration-2 underline-offset-4 hover:text-brand-dark">
                        Ver en Google Maps
                    </a>

                    {/* --- AQUÍ SE AGREGÓ EL MAPA --- */}
                    <div className="w-full h-48 mt-4 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                        <iframe
                            src={sede.embedSrc}
                            width="100%"
                            height="100%"
